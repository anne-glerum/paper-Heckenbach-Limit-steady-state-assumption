...
#include "visco_plastic_strain.h"
...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template<int dim>
    PlasticStrainAdditionalOutputs<dim>::PlasticStrainAdditionalOutputs (const unsigned int n_points)
      :
    ...
    template<int dim>
    std::vector<double>
    PlasticStrainAdditionalOutputs<dim>::get_nth_output(const unsigned int idx) const
    {
...
*/
      ...
      for (unsigned int i=0; i < in.temperature.size(); ++i)
        {
          ...
          if (PlasticStrainAdditionalOutputs<dim> *plastic_out = out.template get_additional_output<PlasticStrainAdditionalOutputs<dim> >())
            {
              double C = 0.;
              double phi = 0.;
    ...
    template <int dim>
    void
    ViscoPlasticStrain<dim>::declare_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    ViscoPlasticStrain<dim>::parse_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    ViscoPlasticStrain<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      if (out.template get_additional_output<PlasticStrainAdditionalOutputs<dim> >() == NULL)
        {
          const unsigned int n_points = out.viscosities.size();
          out.additional_outputs.push_back(
            std_cxx11::shared_ptr<MaterialModel::AdditionalMaterialOutputs<dim> >
            (new MaterialModel::PlasticStrainAdditionalOutputs<dim> (n_points)));
        }
    }
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlasticStrain,
